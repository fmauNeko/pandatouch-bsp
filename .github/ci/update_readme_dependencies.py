    ...
    try:
        ...
        for name, dependency in manifest.dependencies.items():
            ...
            if result:
                ...
                if component != "idf":
                    component = (
                        name if name.startswith("espressif/") else f"espressif/{name}"
                    )
                    component = "[{}]({}{})".format(
                        component, ESP_REGISTRY_URL, component
                    )
...
def get_capabilities_table(header_path, main_path, manifest):
    ...
    with open(header_path, "r") as f:
        h_code = f.read()
        content = h_code.splitlines(keepends=True)
        table_data = []
        for line in content:
            if line.startswith(CAPABILITIES_PREFIX):
                ...
                component_versions = (
                    map_capability_to_driver(capability, manifest) if available else []
                )
                ...
                table_data.append(
                    {
                        ...
                        "Capability": capability_emojis.get(
                            capability, ":black_circle:"
                        )
                        + " "
                        + capability,
...
def check_bsp_readme(bsp_path) -> Any:
    ...
    try:
        table_capabilities_md = (
            markdown_table(table_capabilities_data)
    ...
    pattern = re.compile(rf"{DEPENDENCIES_START}.*?{DEPENDENCIES_END}", re.DOTALL)
    ...
    if os.getenv("GITHUB_ACTIONS") != "true":
        ...
        try:
            ...
            pattern = re.compile(rf"{BENCHMARK_START}.*?{BENCHMARK_END}", re.DOTALL)
...
if __name__ == "__main__":
    os.environ["IDF_VERSION"] = (
        "5.3.0"  # Let's assume IDF v5.3.0 for optional dependencies
    )
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "bsps",
        nargs="*",
        help="BSPs to check. Can be BSP path, BSP README.md, BSP component.yml or BSP header.h file",
    )
